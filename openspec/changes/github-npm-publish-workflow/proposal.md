...
- Use npm trusted publishing through GitHub Actions OIDC instead of a long-lived npm token secret.
- Ensure the workflow uses the project package manager and existing scripts: `build`, `test`, and `typecheck`.
- Document the release/publishing trigger and required npm trusted publisher configuration.
...
- Requires npm package trusted publisher configuration for the GitHub repository and workflow.